<!-- app/components/BlinkMazeGame.vue -->
<template>
  <div
    ref="root"
    class="root"
  ...
  >
    ...
    <div v-if="!isTouch" class="controlsHud desktop">
      ...
      <!-- ✅ Back to Menu -->
      <button class="cBtn" @click="goBack">← Back</button>
    </div>
    ...
    >
      <div class="mTopRow">
        ...
        <button class="mMini" @click="goBack">←</button>
      </div>
...
function generateMaze(seed = Math.floor(Math.random() * 1e9)) {
  ...
  for (let y = 0; y < GRID_H; y++)
    for (let x = 0; x < GRID_W; x++) cellAt(x, y).visited = false
...
function isFromHud(e: PointerEvent) {
  ...
  // keep your old checks + add current class names
  return (
    !!el?.closest?.('.hud') ||
    !!el?.closest?.('.hudMobile') ||
    !!el?.closest?.('.mobileControls') ||
    !!el?.closest?.('.controlsHud')
  )
}
...
function draw() {
  ...
  const cellSize = Math.max(
    12,
    Math.floor(Math.min((w - pad * 2) / GRID_W, (h - pad * 2) / GRID_H))
  )
